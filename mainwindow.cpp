    ...
    : QMainWindow(parent), ui(new Ui::MainWindow), sponsorManager(nullptr), currentSponsorId("")
{
    ...
    if (refreshBtn) {
        connect(refreshBtn, &QPushButton::clicked, this, &MainWindow::on_btn_refresh_stats_clicked);
    }
...
{
    // 1) Chercher les clients dont DATE_AJOUT <= aujourd'hui - 7 jours
    QSqlQuery q;
    if (!q.exec(R"(
        SELECT NOM, PRENOM, ID_CLIENT, DATE_AJOUT
        FROM CLIENT
        WHERE DATE_AJOUT <= SYSDATE - 7
        ORDER BY DATE_AJOUT
    )")) {
        QMessageBox::warning(this, "Erreur SQL",
                             "Impossible de vérifier le suivi :\n" + q.lastError().text());
        return;
    }
    ...
    while (q.next()) {
        ...
        QString pren = q.value(1).toString();
        int id       = q.value(2).toInt();
        QDate d      = q.value(3).toDate();

        lignes << QString("- %1 %2 (#%3, %4)")
                      .arg(nom, pren)
                      .arg(id)
    ...
    if (lignes.isEmpty()) {
        QMessageBox::information(this, "Suivi clients",
                                 "Aucun client à relancer (tous ont moins de 7 jours).");
        return;
    }

    // 2) Construire le corps du SMS
    QString body = "Clients a relancer (7+ jours) :\n" + lignes.join("\n");

    // 3) Envoyer le SMS sur TON numero personnel (verifie chez Twilio)
    QString monNumero = "+21658006157";   // 👈 mets ici ton numero perso vérifié (+216...)

    if (sendSmsTwilio(monNumero, body)) {
        QMessageBox::information(this, "Suivi clients",
                                 "SMS de rappel envoyé sur votre téléphone.");
    } else {
        QMessageBox::warning(this, "Suivi clients",
                             "Échec de l'envoi du SMS (vérifiez Twilio / connexion).");
    }
}
...
void MainWindow::buildClientStatsCharts()
{
    ...
    // 🔥 Regrouper les clients par TYPE
    if (!q.exec("SELECT TYPE, COUNT(*) FROM ABIR.CLIENT GROUP BY TYPE")) {
        QMessageBox::critical(this, "Erreur",
                              "Impossible de charger les statistiques clients :\n" + q.lastError().text());
        return;
    }

    QStringList typeNames;       // ex: entreprise, instagrameur, Autre
    QVector<qreal> clientCounts; // nb de clients par type
    int totalClients = 0;
    ...
    if (typeNames.isEmpty()) {
        QMessageBox::information(this, "Statistiques clients",
                                 "Aucun client trouvé dans ABIR.CLIENT.");
        return;
    }

    // 🧹 Détruire l'ancien graphe s'il existe
    if (m_clientStatsView) {
        delete m_clientStatsView;
        m_clientStatsView = nullptr;
    }

    // ============================
    //  GRAPHE : clients par type
    // ============================
    QBarSet *setClients = new QBarSet("Clients");
    for (qreal v : clientCounts)
        *setClients << v;

    QBarSeries *seriesClients = new QBarSeries();
    seriesClients->append(setClients);

    QChart *chartClients = new QChart();
    chartClients->addSeries(seriesClients);
    chartClients->setTitle("Nombre de clients par type");
    chartClients->setAnimationOptions(QChart::SeriesAnimations);

    // Axe X = types
    QBarCategoryAxis *axisX = new QBarCategoryAxis();
    axisX->append(typeNames);
    chartClients->addAxis(axisX, Qt::AlignBottom);
    seriesClients->attachAxis(axisX);

    // Axe Y = [0 .. max]
    int maxCount = *std::max_element(clientCounts.begin(), clientCounts.end());
    QValueAxis *axisY = new QValueAxis();
    axisY->setRange(0, maxCount);
    axisY->setTitleText("Nombre de clients");
    chartClients->addAxis(axisY, Qt::AlignLeft);
    seriesClients->attachAxis(axisY);

    m_clientStatsView = new QChartView(chartClients);
    m_clientStatsView->setRenderHint(QPainter::Antialiasing);
    m_clientStatsView->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    m_clientStatsView->setMinimumSize(10, 10);  // pour qu'il ne se bloque pas


    // ============================
    //  AJOUT DANS LE WIDGET
    //  (remplace clientStatsContainer par le nom de ton widget)
    // ============================
    QWidget *container = ui->clientStatsContainer;  // 👈 ADAPTE CE NOM SI BESOIN

    if (!container)
        return;

    // Vider l’ancien layout du widget
    ...
    layout->setSpacing(0);
    layout->addWidget(m_clientStatsView);
...
void MainWindow::on_toolButton_5_clicked()
{
    ...
    // 🟦 1) Si champ vide → remettre la table complète
    if (idText.isEmpty()) {
        refreshClientsGrid();
        return;
    }
    ...
    if (!ok || id <= 0) {
        QMessageBox::warning(this, "Recherche", "ID_CLIENT doit être un entier positif.");
        return;
    }

    // 🟦 3) Requête SQL pour filtrer
    QSqlQuery query;
    query.prepare("SELECT * FROM ABIR.CLIENT WHERE ID_CLIENT = :id");
    query.bindValue(":id", id);

    if (!query.exec()) {
        QMessageBox::critical(this, "Erreur SQL", "Erreur lors de la recherche.");
        return;
    }

    // 🟦 4) Vider le tableau avant d'afficher la ligne trouvée
    ui->tab_Client->setRowCount(0);
    ...
    // 🟦 5) Insérer la ligne trouvée dans le tableau
...
void MainWindow::on_btn_export_clients_clicked()
{
    ...
    cursor.insertHtml(R"(
        <div style='text-align:center; margin-bottom:20px;'>
            <h1 style='color:#2266CC; font-size:28px;'>
                📄 Néocreate — Clients Report
            </h1>
        ...
        <hr style='border:1px solid #2266CC; margin:20px 0;'>
    )");
    ...
    tableFormat.setBackground(QColor("#F7FBFF"));

    QTextTable *pdfTable = cursor.insertTable(table->rowCount() + 1, table->columnCount(), tableFormat);

    // En-têtes
    QTextCharFormat headerFormat;
    headerFormat.setBackground(QColor("#2266CC"));
    headerFormat.setForeground(Qt::white);
    headerFormat.setFontWeight(QFont::Bold);

    for (int col = 0; col < table->columnCount(); col++) {
        QTextTableCell cell = pdfTable->cellAt(0, col);
        cell.setFormat(headerFormat);
        cell.firstCursorPosition().insertText(table->horizontalHeaderItem(col)->text());
    }

    // Lignes
    QTextCharFormat rowFormat;
    rowFormat.setFont(QFont("Arial", 10));
    ...
    for (int r = 0; r < table->rowCount(); r++) {
        for (int c = 0; c < table->columnCount(); c++) {
            ...
            pdfTable->cellAt(r + 1, c)
                .firstCursorPosition()
                .insertText(text, rowFormat);
        }
    }
    ...
    // ================================
    cursor.insertHtml(R"(
        <br><hr style='border:1px solid #ccc; margin-top:25px;'>
        <div style='text-align:center; font-size:11px; color:#888;'>
            VoltShop — Generated Automatically • Qt Framework
        </div>
    )");
...
void MainWindow::on_btn_refresh_stats_clicked()
{
    displayProjetStatistics();
    QMessageBox::information(this, "Statistiques", "Les statistiques des projets ont été actualisées.");
}
...
{
    // Update the comprehensive statistics display in tab_14
    displayProjetStatistics();
}
...
void MainWindow::setupProjetStatisticsUI()
{
    ...
    if (ui->tableWidget_stats_type) {
        ui->tableWidget_stats_type->setColumnCount(3);
        QStringList typeHeaders = {"Type", "Nombre", "Pourcentage"};
        ui->tableWidget_stats_type->setHorizontalHeaderLabels(typeHeaders);
        ui->tableWidget_stats_type->horizontalHeader()->setStretchLastSection(true);
        ui->tableWidget_stats_type->setAlternatingRowColors(true);
        ui->tableWidget_stats_type->setSelectionBehavior(QAbstractItemView::SelectRows);
        ui->tableWidget_stats_type->setSortingEnabled(true);
    }

    // Setup statistics tables for payments
    if (ui->tableWidget_stats_paiment) {
        ui->tableWidget_stats_paiment->setColumnCount(4);
        QStringList paimentHeaders = {"Méthode de Paiement", "Nombre", "Montant Total", "Pourcentage"};
        ui->tableWidget_stats_paiment->setHorizontalHeaderLabels(paimentHeaders);
        ui->tableWidget_stats_paiment->horizontalHeader()->setStretchLastSection(true);
        ui->tableWidget_stats_paiment->setAlternatingRowColors(true);
        ui->tableWidget_stats_paiment->setSelectionBehavior(QAbstractItemView::SelectRows);
        ui->tableWidget_stats_paiment->setSortingEnabled(true);
    }
}

void MainWindow::displayProjetStatistics()
{
    // Get statistics data
    ...
    if (ui->label_total_projets) {
        ui->label_total_projets->setText(QString("Total Projets: %1").arg(totalProjects));
    }

    if (ui->label_montant_total) {
        ui->label_montant_total->setText(QString("Montant Total: %1 DT").arg(QString::number(totalMontant, 'f', 2)));
    }

    if (ui->label_montant_moyen) {
        ui->label_montant_moyen->setText(QString("Montant Moyen: %1 DT").arg(QString::number(averageMontant, 'f', 2)));
    }

    // Update type statistics table
    if (ui->tableWidget_stats_type) {
        ui->tableWidget_stats_type->setRowCount(countsByType.size());
        ...
        ui->tableWidget_stats_type->resizeColumnsToContents();
    }

    // Update payment statistics table
    if (ui->tableWidget_stats_paiment) {
        ui->tableWidget_stats_paiment->setRowCount(countsByPaiment.size());
        for (int i = 0; i < countsByPaiment.size(); ++i) {
            ...
            for (const auto &m : montantByPaiment) {
                if (m.first.trimmed().toLower() == paiment.trimmed().toLower()) {
                    montant = m.second;
                    break;
                }
            }
            ...
            ui->tableWidget_stats_paiment->setItem(i, 3, new QTableWidgetItem(QString("%1%").arg(QString::number(percentage, 'f', 1))));
        }
        ui->tableWidget_stats_paiment->resizeColumnsToContents();
    }

    // Create pie charts
    displayProjetPieCharts(countsByType, montantByPaiment, totalProjects, totalMontant);
}

void MainWindow::displayProjetPieCharts(const QVector<QPair<QString,int>>& countsByType,
                                        ...
                                        int totalProjects, double totalMontant)
{
    ...
    if (!ui->groupBox_stats_type || !ui->groupBox_stats_paiment) return;
    ...
    if (totalProjects > 0) {
        ...
        QChartView *typeChartView = new QChartView(typeChart, ui->groupBox_stats_type);
        typeChartView->setRenderHint(QPainter::Antialiasing);
        typeChartView->setGeometry(10, 30, ui->groupBox_stats_type->width() - 20, ui->groupBox_stats_type->height() - 40);
        typeChartView->show();
        m_projetChartView = typeChartView;
    }

    // Create pie chart for payment methods by amount
    if (totalMontant > 0) {
        ...
        QChart *paimentChart = new QChart();
        paimentChart->addSeries(paimentSeries);
        paimentChart->setTitle("Montants par Paiement");
        paimentChart->setTitleFont(QFont("Arial", 12, QFont::Bold));
        paimentChart->legend()->setVisible(false);

        QChartView *paimentChartView = new QChartView(paimentChart, ui->groupBox_stats_paiment);
        paimentChartView->setRenderHint(QPainter::Antialiasing);
        paimentChartView->setGeometry(10, 30, ui->groupBox_stats_paiment->width() - 20, ui->groupBox_stats_paiment->height() - 40);
        paimentChartView->show();
        m_projetChartView2 = paimentChartView;
    }
}
...
void MainWindow::on_comboBox_tri_currentIndexChanged(int index)
{
    qDebug() << "=== TRI COMBOBOX ===";
    qDebug() << "Index:" << index;
    qDebug() << "Texte sélectionné:" << ui->comboBox_tri->currentText();

    // 1. Si tableau vide, on ne peut pas trier
    if (!ui->tableWidget_2 || ui->tableWidget_2->rowCount() == 0) {
        qDebug() << "⚠ Tableau vide ou inexistant";

        // Option: Afficher d'abord les sponsors
        on_AfficherSponsor_clicked();

        // Si toujours vide, on arrête
        if (ui->tableWidget_2->rowCount() == 0) {
            QMessageBox::information(this, "Information",
                                     "Aucun sponsor à trier. Veuillez d'abord ajouter des sponsors.");
            return;
        }
    }

    // 2. Créer objet Sponsor pour utiliser ses fonctions de tri
    Sponsor sponsor;

    // 3. Appliquer le bon tri selon l'index

    if (index == 0) {
        // Option "Tri par Nom"
        qDebug() << "Option 1: Tri par nom alphabétique";
        sponsor.trierParNom(ui->tableWidget_2);
        QMessageBox::information(this, "Tri appliqué", "Sponsors triés par nom (ordre alphabétique)");
    }
    else if (index == 1) {
        // Option "Tri par ID"
        qDebug() << "Option 2: Tri par ID croissant";
        sponsor.trierParId(ui->tableWidget_2);
        QMessageBox::information(this, "Tri appliqué", "Sponsors triés par ID (ordre croissant)");
    }
    else {
        qDebug() << "Option non reconnue:" << index;
    }
