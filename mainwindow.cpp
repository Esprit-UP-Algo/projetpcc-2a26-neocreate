...
#include <QSqlError>
#include <QDate>
#include <QMessageBox>  // pour ClientDAO + sendSmsTwilio
    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    setupTabClient();
    refreshClientsGrid();
    ClientHistory::setListWidget(ui->clientHistoryList);
...
{
    ui->tab_Client->setColumnCount(8);   // 👈 7 → 8 colonnes
    QStringList headers{
        "", "ID_CLIENT", "NOM", "PRENOM",
        "ENTREPRISE", "EMAIL", "TYPE", "DATE_AJOUT"   // 👈 colonne ajoutée
    };
...
Client MainWindow::readClientForm() const
{
    ...
    // 🔥 ICI : on lit toujours la date depuis l’UI,
    // que ce soit ajout ou modification.
    c.DATE_AJOUT = ui->DATE_AJOUT->date();
    // Si tu utilises un QLabel au lieu de QDateEdit:
    // c.DATE_AJOUT = QDate::fromString(ui->DATE_AJOUT->text(), "yyyy-MM-dd");
...
void MainWindow::on_id_confirm_clicked()
{
    ...
    // LECTURE DU FORMULAIRE (inclut maintenant DATE_AJOUT)
    ...
    if (!m_clientEditMode) {
        ...
        // On fixe ici la DATE_AJOUT à aujourd’hui
        c.DATE_AJOUT = QDate::currentDate();     // 👈 AJOUT IMPORTANT
        ...
        // ===== MODIFICATION =====
        // ⚠️ NE PAS toucher DATE_AJOUT → on garde la date originale
...
void MainWindow::on_id_delete_clicked()
{
    ...
    // Récupérer le nom du client à partir du tableau (si présent)
    QString nomClient;
...
void MainWindow::exporterTableWidgetVersHTML()
{
    ...
    if (ui->tableWidget_3->rowCount() == 0) {
        ...
        ui->pdf->setEnabled(true); // Réactiver en cas d'erreur
        return;
    }

    QString fileName = QFileDialog::getSaveFileName(this, "Exporter les employés",
                                                    QDir::homePath() + "/employes.html",
                                                    "Fichiers HTML (*.html)");

    if (fileName.isEmpty()) {
        ui->pdf->setEnabled(true); // Réactiver si annulé
        return;
    }
    ...
    html += "body{font-family:Arial; margin:20px;} h1{color:#2c3e50;text-align:center;}";
    html += "table{border-collapse:collapse;width:100%;} th{background:#34495e;color:white;padding:10px;}";
    html += "td{border:1px solid #ddd;padding:8px;} tr:nth-child(even){background:#f9f9f9;}";
    html += "</style></head><body><h1>LISTE DES EMPLOYÉS</h1>";
    html += "<div style='text-align:center;color:#666;margin:10px 0 20px 0;'>";
    html += "Généré le " + QDateTime::currentDateTime().toString("dd/MM/yyyy à HH:mm");
    html += " | " + QString::number(ui->tableWidget_3->rowCount()) + " employés</div><table>";

    // En-têtes
    html += "<tr><th>ID</th><th>NOM</th><th>PRENOM</th><th>POSTE</th><th>COMPÉTENCE</th><th>TÉLÉPHONE</th><th>EMAIL</th><th>MOT DE PASSE</th></tr>";
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        stream << html;
        file.close();
        // Le bouton sera réactivé par le QTimer
    } else {
...
void MainWindow::afficherStatistiquesDansGroupBox()
{
    ...
    if (ui->tableWidget_3->rowCount() == 0) {
        ...
        ui->groupBox_3->layout()->addWidget(emptyLabel);
        return;
    }
    ...
    if (stats.isEmpty()) {
        ...
        ui->groupBox_3->layout()->addWidget(noDataLabel);
        return;
    }

    QVBoxLayout *mainLayout = qobject_cast<QVBoxLayout*>(ui->groupBox_3->layout());
...
void MainWindow::exportertab_ClientVersHTML()
{
    // Vérifier si le tableau contient des données
    ...
    QString fileName = QFileDialog::getSaveFileName(this, "Exporter les CLIENT",
                                                    QDir::homePath() + "/CLIENT.html",
                                                    "Fichiers HTML (*.html)");
    ...
    // Créer le HTML
    QString html;
    html += "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>CLIENT</title><style>";
    html += "body{font-family:Arial; margin:20px;} h1{color:#2c3e50;text-align:center;}";
    html += "table{border-collapse:collapse;width:100%;} th{background:#34495e;color:white;padding:10px;}";
    html += "td{border:1px solid #ddd;padding:8px;} tr:nth-child(even){background:#f9f9f9;}";
    html += "</style></head><body><h1>LISTE DES CLIENT</h1>";
    html += "<div style='text-align:center;color:#666;margin:10px 0 20px 0;'>";
    html += "Généré le " + QDateTime::currentDateTime().toString("dd/MM/yyyy à HH:mm");
    html += " | " + QString::number(ui->tab_Client->rowCount()) + " CLIENT</div><table>";

    // En-têtes
    html += "<tr><th>ID</th><th>Nom</th><th>Prénom</th><th>Entreprise</th><th>email</th><th>type</th>";

    // Données
    for (int row = 0; row < ui->tab_Client->rowCount(); ++row) {
        html += "<tr>";
        for (int col = 1; col <= 8; ++col) {
            QTableWidgetItem *item = ui->tab_Client->item(row, col);
            QString text = item ? item->text() : "";
            if (col == 8 && !text.isEmpty()) text = "••••••••";
            text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;");
            html += "<td>" + text + "</td>";
