...
bool Projet::AjouterProjet()
{
    ...
    QString sql;
    if (id_client > 0) {
        sql = "INSERT INTO ABIR.PROJET (ID_PROJET, TYPE, DATE_D, DATE_F, MONTANT, PAIMENT, ID_CLIENT) "
              "VALUES (:id, :type, :date_d, :date_f, :montant, :paiment, :id_client)";
    } else {
        sql = "INSERT INTO ABIR.PROJET (ID_PROJET, TYPE, DATE_D, DATE_F, MONTANT, PAIMENT) "
              "VALUES (:id, :type, :date_d, :date_f, :montant, :paiment)";
    }
    query.prepare(sql);
    ...
    query.bindValue(":montant", montant);  // Monetary amount goes to MONTANT column
    query.bindValue(":paiment", paiment);   // Payment method goes to PAIMENT column
...
QSqlQueryModel* Projet::AfficherProjets()
{
    ...
    // Format dates as ISO text for consistent UI rendering, include both MONTANT and PAIMENT
    model->setQuery(
...
// Afficher la liste des projets triés
QSqlQueryModel* Projet::AfficherProjetsTri\u00e9s(const QString &sortBy)
{
    ...
    if (sortBy == "Date début" || sortBy == "date_debut") {
        query += "ORDER BY DATE_D";
    } else if (sortBy == "Date fin" || sortBy == "date_fin") {
        query += "ORDER BY DATE_F";
    } else if (sortBy == "Montant" || sortBy == "montant") {
        query += "ORDER BY MONTANT DESC"; // DESC for amount makes more sense
    } else {
        query += "ORDER BY ID_PROJET"; // Default sort by ID
    }
    ...
    if (model->lastError().isValid()) {
        qWarning() << "[AfficherProjetsTri\u00e9s] Query error:" << model->lastError().text();
    } else {
        qDebug() << "[AfficherProjetsTri\u00e9s] Query set successfully with sort:" << sortBy;
    }
...
QSqlQueryModel* Projet::ChercherProjetParId(int id)
{
    ...
    if (model->lastError().isValid()) {
        qWarning() << "[ChercherProjetParId] Query error:" << model->lastError().text();
    } else {
        qDebug() << "[ChercherProjetParId] Query set successfully for ID:" << id;
    }

    return model;
}

// Méthode statique pour chercher un projet par ID
QSqlQueryModel* Projet::ChercherProjetParIdStatique(int id)
{
    ...
    if (model->lastError().isValid()) {
        qWarning() << "[ChercherProjetParIdStatique] Query error:" << model->lastError().text();
    } else {
        qDebug() << "[ChercherProjetParIdStatique] Query set successfully for ID:" << id;
    }
...
bool Projet::ModifierProjet()
{
    ...
    QString sql;
    if (id_client > 0) {
        sql = "UPDATE ABIR.PROJET SET TYPE = :type, DATE_D = :date_d, DATE_F = :date_f, MONTANT = :montant, PAIMENT = :paiment, ID_CLIENT = :id_client WHERE ID_PROJET = :id";
    } else {
        sql = "UPDATE ABIR.PROJET SET TYPE = :type, DATE_D = :date_d, DATE_F = :date_f, MONTANT = :montant, PAIMENT = :paiment, ID_CLIENT = NULL WHERE ID_PROJET = :id";
    }
    query.prepare(sql);
